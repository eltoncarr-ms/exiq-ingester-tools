...
Parsed `stage`/`cycle` events are grouped with `buildPollAnalysis` into per-source dashboards (cycle outcomes,
throughput, and stage totals), source-throughput trends, a cycle list scoped to the selected source or, when cycles
still have stage activity but no rollup yet (no source assigned), to a distinct **Source pending** selection — never
both at once — and interactive stage-timeline replay for the selected cycle. The selected source (or pending) and
cycle stay stable as new events arrive; the header surfaces a running warning count for malformed or unrecognized
lines and any load error.
